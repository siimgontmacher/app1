...
template = """
 You are a marketing copywriter with 20 years of experience. You are analyzing customer's background to write personalized product description that only this customer will receive; 
    ...
    USE CASE: write a story in 5 sentences, of an example weekend activity taking into account size {size} and age {agegroup}, write a story in first person, example "I started my Saturday morning with ..."; OUTPUT TEXT in Estonian; 
"""
...
def get_hobby():
    input_text = st.text_input(label="Customers size", key="hobby_input")
    return input_text
...
#        st.stop()
    ...
    prompt_with_content = prompt.format(agegroup=option_agegroup, size=hobby_input, content=content_input)
