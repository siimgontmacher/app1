...
template = """
 You are a marketing copywriter with 20 years of experience. You are analyzing customer's background to write personalized product description that only this customer will receive; 
    ...
    CUSTOMER main Size: {size};
    TASK: Write a product description that is tailored into this customer's Age group and size. Use age group specific slang.;
    ...
    BENEFITS: describe in 3 sentences why this product is perfect considering customers age group and size;
    USE CASE: write a story in 5 sentences, of an example working day profession taking into account size {size} and age {agegroup}, write a story in first person, example "I started my workday morning with ...";
"""

prompt = PromptTemplate(
    input_variables=["agegroup", "size", "content"],
    template=template,
)
...
def load_LLM(openai_api_key):
    ...
    llm = OpenAI(model_name='gpt-3.5-turbo-instruct', temperature=.9, openai_api_key=openai_api_key)
    return llm
...
with col1:
    st.markdown("Otstarve: tootetutvustustekstide personaliseerimine igale kliendile või kliendigruppidele; väljundtekst on kohandatud kliendi a) vanuserühmaga ja b) suurusega; sisendtekstiks on neutraalses vormis tootekirjeldus. \
    \n\n Kasutusjuhend: 1) valmista ette tootekirjeldus (sisendtekst). 2) määra tarbijasegemendid lähtuvalt vanuserühma ja suuruse kombinatsioonidest. 3) sisesta ükshaaval tarbijasegmentide lõikes eeltoodud info äpi kasutajaliideses, saada ära. \
    4) kopeeri ükshaaval tarbijasegmentide lõikes äpi väljundteksti kõnealuse toote tutvustuslehele.")

with col2:
    st.image(image='companylogo.jpg', caption='Natural and healthy suits for everybody')
...
with col1:
    option_agegroup = st.selectbox(
        ...
        ('16-20', '21-30', '31-40', '41-60', '61-80', '81-100'))
    
def get_size():
    input_text = st.text_input(label="Customers main size", key="size_input")
    return input_text
...
def update_text_with_example():
    print ("in updated")
    st.session_state.content_input = "suits, all clolors, cotton, responsible manufacturing"
